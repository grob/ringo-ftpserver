...
/** @ignore */
FtpServer.FTPLET_NAME = "_eventDispatcher";
...
FtpServer.prototype.bind = function(eventName, callback) {
    if (!this.ftplets.containsKey(FtpServer.FTPLET_NAME)) {
        this.addFtplet(FtpServer.FTPLET_NAME, this.events);
    }
 ...
 * @param {String|Object} fileOrObj Either the path to the JSON file containing
 * the user accounts, or an object containing the account data.
...
FtpServer.createUserManager = function(fileOrObj) {
    ...
    if (typeof(fileOrObj) === "string") {
        impl = new JsonUserManager(fileOrObj);
    } else if (fileOrObj.constructor === Object) {
        impl = new UserManager(fileOrObj);
    } else {
        throw new Error("createUserManager requires a file path or object as argument");
    }
    return new org.apache.ftpserver.ftplet.UserManager(impl);
};
